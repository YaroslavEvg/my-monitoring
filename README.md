...
| `encoding_file` | `utf-8` | Целевая кодировка текстовых файлов перед упаковкой в zip (если перекодировка невозможна — кладём байты как есть). |
...
| `file.path`, `file.field_name`, `file.content_type` | ✖ | Настройки отправки локального файла в multipart/form-data. Если путь — не zip или каталог, агент упакует содержимое в zip и отправит его как `application/zip`. При упаковке попытаться перекодировать текстовые файлы в `encoding_file`. |
| `encoding_file` | ✖ | Целевая кодировка текстовых файлов перед упаковкой в zip (по умолчанию `utf-8`). |
...
Если одновременно требуется отправить файл и JSON (multipart/form-data), укажите файл в секции `file`, а JSON — как обычно. Монитор соберёт multipart в формате, который работает с требуемым бэкендом: JSON передаётся отдельной частью `application/json` **без** `filename`, файл — обычной частью с именем файла. Это единственный поддерживаемый вариант загрузки файлов. Опция `multipart_json_filename` больше не используется: JSON отправляется без имени файла. При необходимости можно переименовать поле JSON через `multipart_json_field` (по умолчанию `json`) и задать кодировку JSON-части через `encoding_json` (по умолчанию `utf-8`). Если `file.path` указывает на не-zip файл, агент автоматически упакует его в zip перед отправкой; если указан каталог, в zip попадёт сама папка и все вложенные файлы. Перед добавлением в zip агент пытается перекодировать текстовые файлы в `encoding_file` (по умолчанию `utf-8`), если это не удаётся — кладёт байты как есть. Имя архива формируется как `<имя_файла_без_расширения>.zip` или `<имя_папки>.zip`. Архив создаётся в отдельной временной папке перед каждым запросом и удаляется после него.
