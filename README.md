...
| `delay_before` | ✖ | Пауза перед выполнением запроса (секунды). |
| `children_delay` | ✖ | Пауза между родителем и его детьми (если у ребёнка не задан `delay_before`). |
| `wait_for.path`, `wait_for.attempts`, `wait_for.delay` | ✖ | Ожидание появления JSON-поля: путь, число попыток, пауза между попытками. |
...
| `env` | ✖ | Локальные переменные окружения для маршрута (перекрывают глобальные `env`). |
...
Можно подставлять значения из переменных окружения через синтаксис `${VAR}`. Также можно объявить
блок `env` в конфиге — его значения доступны всем маршрутам из файла, а блок `env` внутри маршрута
...
Для выборки элемента из списка поддерживается фильтр по значению: `$.items[key4=val4].key5` или
`$.items[key4="val4"].key5`. Можно задавать несколько условий через `&`:  
`$.[tech.techName="test"&fields[name="python"].value="need_value"].uuid`.  
В результате выбирается первый элемент списка, где все условия совпали.

Синтаксис фильтра: `$.list[условие].field`, где `условие` — это `path=value` (или `path==value`).
`path` может быть вложенным (`meta.code`) и содержать фильтр внутри (`fields[name="python"].value`).
Для корневого массива используйте `$.[...]`. Если совпадений нет, значение не подставляется.

Примеры фильтров:
- `$.items[0].id` — индекс в списке.
- `$.[0].id` — индекс в корневом массиве.
- `$.items[key=val].id` — значение без кавычек.
- `$.items[key="val"].id` — строка в кавычках.
- `$.items[key==val].id` — эквивалентно `=`.
- `$.items[key=123].id` — числовое значение.
- `$.items[key=true].id` — булево значение.
- `$.items[key=null].id` — значение `null`.
- `$.items[key="a&b"].id` — значение с `&` (в кавычках).
- `$.items[key1=val1&key2=val2].id` — несколько условий (логическое И).
- `$.items[meta.code=200&meta.type="ok"].id` — условия по вложенным полям.
- `$.items[fields[name="python"].value="need_value"].uuid` — поиск по вложенному списку.
- `$.[tech.techName="test"&fields[name="python"].value="need_value"].uuid` — фильтр в корневом массиве.

В результатах сохраняется только один запрос из цепочки: первый с невалидным кодом ответа (`ok=false`),
а если все успешны — последний дочерний запрос. Время (`response_time_ms`) — сумма запросов и ожиданий в цепочке.

Чтобы добавить паузу перед запросом, используйте `delay_before`. Для задержки между родителем и детьми
задайте `children_delay` у родителя — он применяется к детям, если у них нет собственного `delay_before`.
Для ожидания появления поля в JSON-ответе используйте `wait_for`: запрос будет повторён до `attempts` раз
с паузой `delay` между попытками. Если поле не найдено, запрос помечается как `ok=false`.
