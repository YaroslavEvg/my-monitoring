...
Если переменные удобнее хранить в `.env`, укажите один или несколько файлов через `--env-file`.
Формат — `KEY=VALUE` (опционально `export`), поддерживаются кавычки, комментарии `#` и ссылки
на другие переменные через `${VAR}`. Значения из `.env` попадают в окружение ещё до загрузки конфига.
